...
import { Mic, MicOff, Search, AlertCircle, ExternalLink, LayoutGrid, X, Clock, ChevronDown, ChevronRight, Globe, MapPin, Trash2, Bug, Terminal, Brain, FileText, Upload, FilePlus, Cloud, CloudOff, User, Settings, Copy, Check, MonitorPlay, Smile, Frown } from 'lucide-react';

import { Avatar3D } from './components/Avatar3D';
import { Loader } from './components/Loader';
import { useGeminiLive } from './hooks/useGeminiLive';
import { useGoogleDrive } from './hooks/useGoogleDrive';
import { ConnectionState, GroundingChunk, GroundingMetadata, Memory, WorkspaceFile } from './types';
...
const App = () => {
  ...
  // Google Drive
  const { login, logout, user: googleUser, accessToken, loadMemories: loadDriveMemories, saveMemories: saveDriveMemories, isSyncing, clientId, setClientId } = useGoogleDrive();
  ...
  useEffect(() => {
      // 1. Try loading from Drive if logged in
      if (accessToken) {
          loadDriveMemories().then(driveMemories => {
              if (driveMemories) {
                  setMemories(driveMemories);
                  console.log("Loaded memories from Drive");
              }
          });
      } else {
      ...
      try {
          ...
          if (storedHist) {
              ...
              setSearchHistory(parsed.map((h: any) => ({ ...h, timestamp: new Date(h.timestamp) })));
          }
  ...
  }, [accessToken, loadDriveMemories]);

  // --- PERSISTENCE EFFECTS ---
  useEffect(() => { 
      // Save to Drive if logged in
      if (accessToken) {
          const timeout = setTimeout(() => {
              saveDriveMemories(memories);
          }, 2000); // Debounce saves
          return () => clearTimeout(timeout);
      } 
      // Always save to LocalStorage as backup
      localStorage.setItem('gem_long_term_memory', JSON.stringify(memories)); 
  }, [memories, accessToken, saveDriveMemories]);
  ...
  const triggerGesture = useCallback((gestureName: string) => {
      ...
      // Clear gesture after 2.5s to allow re-triggering
  ...
  const clearMemories = () => { 
      setMemories([]); 
      localStorage.removeItem('gem_long_term_memory'); 
      if(accessToken) saveDriveMemories([]);
  };
  ...
  return (
    <div className="relative w-full h-screen bg-[#0a0a12] overflow-hidden font-sans">
      ...
      {/* Header UI - Left Vertical Stack (Icon Only) */}
              ...
              {accessToken ? (
                  <div className="bg-slate-800/50 rounded-lg p-3 border border-white/5 flex items-center justify-between">
                      <div className="flex items-center gap-2">
                          ...
                          <span className="text-xs text-slate-300 truncate max-w-[100px]">{googleUser?.name || 'Google User'}</span>
                      </div>
                      <div className="flex items-center gap-2">
                          {isSyncing && <Cloud size={14} className="text-indigo-400 animate-pulse" />}
                          <button onClick={logout} className="text-[10px] bg-white/5 hover:bg-white/10 px-2 py-1 rounded text-slate-400">Sign Out</button>
                      </div>
                  </div>
              ) : (
                  <button onClick={handleLogin} className="w-full bg-white/5 hover:bg-white/10 border border-white/10 rounded-lg p-2 flex items-center justify-center gap-2 text-xs text-slate-300 transition-colors">
                      <CloudOff size={14} />
                  ...
                  memories.map((memory) => (
                      <div key={memory.id} className="bg-slate-900/60 border border-white/5 rounded-xl p-3 group relative">
                          ...
                          <div className="flex items-center justify-between mt-2 pt-2 border-t border-white/5">
                              ...
                              {accessToken && <Cloud size={10} className="text-indigo-400/50" />}
                          </div>
                      ...
                      <div key={file.id} className="bg-slate-900/60 border border-white/5 rounded-xl overflow-hidden">
                           ...
                           {expandedFileId === file.id && (
                               <div className="bg-black/30 border-t border-white/5 p-3">
                                   <div className="flex justify-end mb-2"><button onClick={() => deleteFile(file.id)} className="text-xs text-red-400 hover:text-red-300 flex items-center gap-1"><Trash2 size={12} /> Delete</button></div>
      ...
      <div className={`absolute top-0 right-0 h-full w-96 bg-slate-950/95 backdrop-blur-xl border-l border-white/10 shadow-2xl transform transition-transform duration-300 ease-out z-50 flex flex-col ${isSearchDrawerOpen ? 'translate-x-0' : 'translate-x-full'}`}>
          <div className="p-6 border-b border-white/10 flex justify-between items-center bg-slate-900/50">
              ...
              <div className="flex items-center gap-2">
                {searchHistory.length > 0 && <button onClick={() => {setSearchHistory([]); localStorage.removeItem('gem_search_history')}} className="text-slate-400 hover:text-red-400 transition-colors p-2 hover:bg-white/10 rounded-full"><Trash2 size={16} /></button>}
                          ...
                          {expandedHistoryIds.has(item.id) && (
                              <div className="bg-black/20 px-3 py-3 space-y-2 border-t border-white/5">
                                  {item.sources.map((source, idx) => (
                                      <a key={idx} href={source.uri} target="_blank" rel="noopener noreferrer" className="flex items-start gap-2.5 p-2 rounded-lg hover:bg-white/5 group">
                                          <div className="mt-0.5 bg-slate-800 p-1 rounded text-indigo-400">{source.type === 'map' ? <MapPin size={12} /> : <Globe size={12} />}</div>
      ...
      {(isSearchDrawerOpen || isMemoryDrawerOpen || isWorkspaceDrawerOpen) && <div className="absolute inset-0 bg-black/60 backdrop-blur-sm z-40" onClick={() => { setIsSearchDrawerOpen(false); setIsMemoryDrawerOpen(false); setIsWorkspaceDrawerOpen(false); }} />}
          ...
          <div className="absolute inset-0 bg-black/80 backdrop-blur-md z-[60] flex items-center justify-center p-4">
              <div className="bg-slate-900 border border-white/10 rounded-2xl shadow-2xl max-w-md w-full p-6 animate-in fade-in zoom-in-95 duration-200">
                  <div className="flex items-center gap-3 mb-4">
                      ...
                      <h2 className="text-xl font-bold text-white">Configure Google Drive</h2>
                  </div>
      ...
      {showSources && !isSearchDrawerOpen && groundingMetadata && groundingMetadata.groundingChunks?.length > 0 && (
